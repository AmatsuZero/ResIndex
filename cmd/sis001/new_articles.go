...
func NewArticle() *cobra.Command {
	cmd := &cobra.Command{
		...
		PreRun: func(cmd *cobra.Command, args []string) {
			err := dao.DB.AutoMigrate(&dao.SisPageModel{})
			if err != nil {
...
func (n *NewList) extractDetails(infos []ThreadInfo) []*dao.SisPageModel {
	wg := sync.WaitGroup{}

	output := make([]*dao.SisPageModel, 0)
	var lock sync.Mutex
...
func (n *NewList) extractDetail(info ThreadInfo) (*dao.SisPageModel, error) {
	detail := Detail{
		ThreadInfo: info,
		Category:   "new",
		Host:       n.host,
